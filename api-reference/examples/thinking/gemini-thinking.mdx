...
`thinkingLevel` 参数用于控制模型在生成回答之前进行内部推理过程的最大深度。Gemini 3 将这些级别视为相对的思考许可，而不是严格的令牌保证。如果未指定 `thinkingLevel`，Gemini 3 Pro 将默认为 `HIGH`。
...
您不能在同一请求中同时使用 `thinkingLevel` 和旧版 `thinkingBudget` 参数。这样做会返回 400 错误。
</Warning>
