...
### `/v1/images/edits` 特有参数

| 参数 | 类型 | 说明 |
|---|---|---|
| `image` | file / file[]（form-data）<br/>string / string[]（JSON，URL 或 base64） | 参考图。JSON 下多图就是数组；form-data 下同名字段重复即可多图 |
| `mask` | file（form-data）<br/>string（JSON，URL 或 base64） | 可选 PNG mask（alpha 通道透明区域表示"编辑此处"），尺寸必须与 `image` 一致 |
...
- 常用值：`1024x1024 / 1536x1024 / 1024x1536 / 4K`
- 超出约束会被上游 400 拒绝
