...
JSON 请求中的 `image` 是字符串数组，每个元素可以是 HTTP(S) URL、纯 Base64，或带 `data:image/...;base64,` 前缀的 Base64。可选的 `mask` 是单个 URL 或 Base64 字符串。
...
| 参数 | 类型 | 说明 |
|---|---|---|
| `model` | string | `gpt-image-2.5-flare` 或 `gpt-image-2.5-sunburst` |
| `prompt` | string | 图片描述，必填 |
| `size` | string | `auto` 或 `WIDTHxHEIGHT`；推荐使用 `1024x1024`、`1536x1024`、`1024x1536` |
| `quality` | string | `low` / `medium` / `high` / `xhigh` / `max` / `auto`；质量越高，通常耗时和输出 token 越多 |
| `output_format` | string | `png`（默认）/ `jpeg`；当前平台接入暂不支持 `webp` |
| `output_compression` | integer | `0` ~ `100`；当前仅在 `output_format=jpeg` 时使用，不传则使用模型默认值 |
| `background` | string | `auto` / `opaque` / `transparent`；透明背景请使用 `png` |
| `moderation` | string | `auto`（默认）/ `low` |
| `input_fidelity` | string | 图片编辑参数；`high` 仅由 `gpt-image-2.5-sunburst` 支持，Flare 请省略 |
| `response_format` | string | `b64_json`（默认）/ `url` |
...
| 参数 | 类型 | 说明 |
|---|---|---|
| `image` | file / file[]（form-data）<br/>string / string[]（JSON，URL 或 base64） | 参考图。JSON 下多图就是数组；form-data 下同名字段重复即可多图。GPT Image 2.5 与 `gpt-image-1/2` 家族在 `/v1/images/generations` 也支持此字段（见方式 C） |
