...
Seedream 5.0 Pro（`doubao-seedream-5-0-pro-260628`）和 Seedream 5.0 lite（`doubao-seedream-5-0-260128`）相比 4.0 版本新增了以下能力。Pro 版本面向高精度图片生成场景，调用方式与 lite 一致，只需替换 `model` 字段。

### 指定输出格式（output_format）

5.0 Pro / lite 支持 `png` 和 `jpeg` 两种输出格式（4.0/4.5 仅支持 jpeg）。

<CodeGroup>

```bash cURL - 指定 PNG 格式输出
curl -X POST "https://model-api.skyengine.com.cn/v1/images/generations" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <API-KEY>" \
  -d '{
...
- 联网搜索会增加一定的响应时延
- 仅 Seedream 5.0 Pro / lite 支持，4.0/4.5 不支持
</Info>
...
|------|----------|--------|----------|----------|
| Seedream 5.0 Pro | `doubao-seedream-5-0-pro-260628` | 1K, 2K | png, jpeg | 支持 |
| Seedream 5.0 lite | `doubao-seedream-5-0-260128` | 2K, 3K, 4K | png, jpeg | 支持 |
...
- **model**: 模型名称
- **n**: 生成图片数量。Seedream 5.0 Pro 当前仅支持生成单图
- **size**: 图片尺寸，分辨率等级（如 `"1K"`、`"2K"`、`"3K"`）或精确像素值（如 `"2048x2048"`）。Seedream 5.0 Pro 支持 `1K`、`2K`
- **response_format**: 返回格式（`url` 或 `b64_json`）
- **output_format**: 输出图片格式（`png` 或 `jpeg`），仅 5.0 Pro / lite 支持 png
- **seed**: 随机数种子，-1 表示随机
...
- **tools**: 工具列表，如 `[{"type": "web_search"}]` 开启联网搜索（仅 5.0 Pro / lite）
- **sequential_image_generation**: 组图功能控制（`auto` 或 `disabled`）
- **sequential_image_generation_options**: 组图功能配置
  - **max_images**: 最大图片数量
- **Seedream 5.0 Pro 注意事项**: `doubao-seedream-5-0-pro-260628` 不支持 `sequential_image_generation` / `sequential_image_generation_options`，调用 Pro 时请不要传这两个字段
- **optimize_prompt_options**: 提示词优化配置
  - **mode**: 优化模式（`standard`）
