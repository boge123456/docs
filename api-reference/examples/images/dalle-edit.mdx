---
title: 'OpenAI 图片编辑示例'
description: '使用OpenAI gpt-image-1 进行图片编辑和变体生成的完整示例代码'
---
...
```bash cURL - 图片变体
curl -X POST "https://model-api.skyengine.com.cn/v1/images/variations" \
  -H "Authorization: Bearer <API-KEY>" \
  -F image="@original.png" \
  -F n=2 \
  -F size="1024x1024"
...
def create_image_variations(image_path, n=2, size="1024x1024"):
    """
    生成图片变体 - 基于原图生成相似但不同的图片
...
if __name__ == "__main__":
    ...
    if os.path.exists(image_file):
        ...
        # 示例2: 图片变体
        print("2. 图片变体示例:")
        variation_result = create_image_variations(
            image_path=image_file,
            n=2,
            size="1024x1024"
        )
        ...
        if 'error' not in variation_result:
            print("变体生成成功！")
            for i, img_data in enumerate(variation_result):
                print(f"- 变体图片 {i+1}: {img_data['url']}")
                # 下载图片
                download_image(img_data['url'], f"variation_{i+1}.png")
        else:
            print(f"变体生成失败: {variation_result['error']}")
...
async function editImage(imagePath, maskPath, prompt, n = 1, size = '1024x1024') {
...
async function createImageVariations(imagePath, n = 2, size = '1024x1024') {
...
(async () => {
    try {
        ...
        if (fs.existsSync(imageFile)) {
            ...
            // 示例2: 图片变体
            console.log('2. 图片变体示例:');
            const variationResult = await createImageVariations(imageFile, 2, '1024x1024');

            if (!variationResult.error) {
                console.log('变体生成成功！');
                for (let i = 0; i < variationResult.length; i++) {
                    console.log(`- 变体图片 ${i + 1}: ${variationResult[i].url}`);
                    await downloadImage(variationResult[i].url, `variation_${i + 1}.png`);
                }
            } else {
                console.log(`变体生成失败: ${variationResult.error}`);
            }
        } else {
            console.log(`请先准备示例图片文件: ${imageFile}`);
            console.log('图片要求:');
            console.log('- 格式: PNG');
...
|------|------|------|------|
| `image` | file | 是 | 原始图片文件 |
| `mask` | file | 编辑时必需 | 遮罩图片文件 |
| `prompt` | string | 编辑时必需 | 编辑描述文字 |
