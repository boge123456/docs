...
配置当前识别会话。平台的火山引擎适配器会解析该事件。该事件应作为连接后的第一条消息发送；服务端返回 `session.ready` 后再发送音频。
...
不发送 `session.config`、直接发送第一条 `input_audio_buffer.append` 时，平台会使用模型默认配置。
...
| `trace_id` | string | 否 | 用于关联当前音频片段和识别事件。 |
