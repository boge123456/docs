...
curl --request POST \
  ...
  --data '{
    ...
    "content": [
        {
            "type": "text",
            "text": "一位音乐家在舞台上演奏小提琴，观众席上掌声雷动 --ratio 16:9 --duration 8"
        }
    ],
    "generate_audio": true
  }'
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| **type** | String | 是 | 内容类型：`text`（文本）、`image_url`（图片）、`audio_url`（音频） |
...
| **seconds** | String | 否 | 视频时长（秒），范围取决于模型：`doubao-seedance-1-0-pro-250528` 支持 2~12秒，`doubao-seedance-1-5-pro-251215` 支持 4~12秒 |
...
```json
{
  ...
  "content": [
    {
      "type": "text",
      "text": "一位音乐家在舞台上演奏小提琴，观众席上掌声雷动。镜头聚焦音乐家的手指在琴弦上舞动，音乐厅的灯光洒在他专注的脸上。 --ratio 16:9 --duration 8"
    }
  ],
  "generate_audio": true
}
