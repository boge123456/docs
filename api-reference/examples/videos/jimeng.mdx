...
def create_omnihuman_video(image_url, audio_url, prompt="", model="OmniHuman_1.5", **kwargs):
    """
...
async function createOmniHumanVideo(imageUrl, audioUrl, prompt = '', model = 'OmniHuman_1.5', options = {}) {
    const url = `${BASE_URL}/videos`;
