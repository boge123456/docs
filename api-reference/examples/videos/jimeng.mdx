...
def detect_subject(image_url, req_key="jimeng_realman_avatar_object_detection"):
    """
...
async function detectSubject(imageUrl, reqKey = 'jimeng_realman_avatar_object_detection') {
...
func detectSubject(imageURL string, reqKey string) (*SubjectDetectionResponse, error) {
    if reqKey == "" {
...
def recognize_subject(image_url, req_key="jimeng_realman_avatar_picture_create_role_omni_v15"):
    """
...
async function recognizeSubject(imageUrl, reqKey = 'jimeng_realman_avatar_picture_create_role_omni_v15') {
...
func recognizeSubject(imageURL string, reqKey string) (*SubjectRecognitionResponse, error) {
    if reqKey == "" {
...
def create_omnihuman_video(image_url, audio_url, prompt="", model="jimeng_realman_avatar_picture_omni_v15", **kwargs):
    """
...
async function createOmniHumanVideo(imageUrl, audioUrl, prompt = '', model = 'jimeng_realman_avatar_picture_omni_v15', options = {}) {
    const url = `${BASE_URL}/videos`;
...
func createOmniHumanVideo(imageURL, audioURL, prompt, model string, outputResolution int, peFastMode bool) (*VideoResponse, error) {
    url := fmt.Sprintf("%s/videos", BaseURL)
...
| 参数 | 类型 | 必选 | 说明 |
|------|------|------|------|
| **model** | String | 是 | 数字人模型名称，如 `jimeng_realman_avatar_picture_omni_v15` |
