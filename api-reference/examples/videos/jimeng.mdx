...
- **aspect_ratio**: 生成视频的长宽比，默认"16:9"，支持常见比例
...
### 技术优势
- **高质量生成**: 即梦Ti2V v3.0 Pro版本，专业级视频生成质量
- **中文优化**: 对中文提示词有优秀的理解和表现能力
- **灵活参数**: 支持自定义帧数、宽高比、随机种子等参数
- **图片引导**: 支持参考图片生成，保持风格一致性

### 生成规格
- **帧数选择**: 121帧（约5秒）或241帧（约10秒）
- **分辨率**: 支持多种分辨率，自动优化
- **宽高比**: 16:9, 9:16, 1:1等多种比例
- **文件格式**: MP4格式输出
...
```text
推荐的提示词格式：
"[主体描述] + [动作描述] + [场景环境] + [风格要求] + [画质要求]"

优秀示例：
"一只白色的小猫在绿色草地上慢慢走动，阳光透过树叶洒下，温馨治愈风格，高清画质"
```

### 2. 参数调整建议
- **seed**: -1获得随机效果，固定值获得一致结果
- **frames**: 121帧适合短片段，241帧适合完整场景
- **aspect_ratio**: 根据发布平台选择合适比例 
...
### 4. 图片裁剪规则
参考：https://www.volcengine.com/docs/85621/1777001#eozh1inS

## 注意事项

1. **生成时间**: 即梦视频生成通常需要3-5分钟，请耐心等待
2. **内容审核**: 提示词需符合内容安全规范
3. **并发限制**: 同时生成任务数量有限制
4. **中文优化**: 中文提示词效果通常优于英文
