...
Claude Code 是 Anthropic 推出的专业编码助手，功能强大但使用门槛较高。不仅需登录专属 Claude 账号，还需购买官方月度订阅才能解锁。为了让 ModelHub 平台用户更便捷的享受这一工具，我们已完成对 Claude Code 进行兼容，您可通过下列方式安装使用。
...
Claude Code 需要配置环境变量来连接 ModelHub 服务：
- `ANTHROPIC_BASE_URL` - API 接入点
- `ANTHROPIC_AUTH_TOKEN` - ModelHub 平台的API-Key
...
```bash
# ModelHub Claude Code配置(必填项)
export ANTHROPIC_BASE_URL="https://model-api.skyengine.com.cn"
...
- 变量值：`https://model-api.skyengine.com.cn`
...
**变量 3：**
- 变量名：`ANTHROPIC_MODEL`
- 变量值：`Claude-xxxxxxxx`（替换为 ModelHub 平台的实际模型名称）
...
恭喜您！所有配置都已完成。Claude Code 会启动一个交互式的 REPL 会话，您可以直接开始提问和交互。由于我们已经配置好了 API 接入点和 Key，它现在完全通过 ModelHub 平台运行。
